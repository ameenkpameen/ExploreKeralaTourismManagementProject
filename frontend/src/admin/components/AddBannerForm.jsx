...
function AddBannerForm() {
    ...
    const addsBanner = useSelector(state => state.ownerAddBanner)
    const { loading,error} = addsBanner
    ...
    const imageUpload = (async (e)=>{
      ...
      const submitHandler = async (e) => {
          e.preventDefault();
          console.log(heading, description, image_url, public_id,productId,id,type);
          dispatch(OwnerAddBanner(heading, description, image_url, public_id,productId,id,type));
          setOpen(true)
        };
    ...
    <div>
           <div className="container mx-auto py-8">
                ...
                <form onSubmit={submitHandler} className="w-full max-w-sm mx-auto text-white bg-[#20354b] p-8 rounded-lg shadow-xl drop-shadow-2xl" encType="multipart/form-data">
                    <div className="mb-4">
                        <label className="block text-white text-sm font-bold mb-2" htmlFor="destination">Heading</label>
                        <input className="w-full px-3 py-2 border text-black border-gray-300 rounded-md focus:outline-none focus:border-indigo-500"
                        type="text" id="name" name="destination" placeholder="" value={heading} onChange={(e)=>setHeading(e.target.value)}/>
                    </div>
                    <div className="mb-4">
                        <label className="block text-white text-sm font-bold mb-2" htmlFor="district">Description</label>
                        <input className="w-full px-3 py-2 border text-black border-gray-300 rounded-md focus:outline-none focus:border-indigo-500"
                        type="text" id="email" name="district" value={description} onChange={(e)=>setDescription(e.target.value)} />
                    </div>
                    <div className="mb-4">
                        <label className="block text-white text-sm font-bold mb-2" htmlFor="district">Type</label>
                        <select className='w-full px-3 py-2 border text-black border-gray-300 rounded-md focus:outline-none focus:border-indigo-500' onChange={(e)=>setType(e.target.value)}>
                        ...
                        {/* <input className="w-full px-3 py-2 border text-black border-gray-300 rounded-md focus:outline-none focus:border-indigo-500"
                        type="text" id="email" name="district" value={type} onChange={(e)=>setType(e.target.value)} /> */}
                    </div>
                    <div className="mb-4">
                        <label className="block text-white text-sm font-bold mb-2" htmlFor="district">Property</label>
                        <div className='w-full px-3 py-2 mb-3 border text-black border-gray-300 rounded-md focus:outline-none focus:border-indigo-500'>
                                        <select
                                          type="text"
                                        //   value={firstname}
                                          name="type"
                                            ...
                                            <option value="" className='text-gray-400'>Choose</option>
                                                {productData && productData.map((element, index)=>(
                                                    <option value={element._id}>
                                                        ...
                                                        <p>{element.brandname+element.modelname}</p>
                                                    }
                                                    </option>
                                                ))}
                  ...
                  <div className='text-center w-56'>
                        ...
                        <div>
                          ...
                          <div>
                            ...
                            <div className='mx-auto my-4 w-48'>
                                <h3 className='text-lg font-black text-gray-800 '>Banner Added</h3>
                                <p className='text-sm text-red-600'>Successfully</p>
                            </div>
                            <div className="flex gap-4">
                                <button onClick={()=>{navigate('/owner');setOpen(false)}} className="btn bg-white text-black shadow-lg hover:bg-gray-100 p-2 rounded-lg w-full">Ok</button>
                            </div>
                          </div>
